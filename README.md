...
The Auto EDA Analyst now profiles local CSV files into deterministic Markdown with a concise analyst summary, configurable validated one-character delimiters for non-comma exports, complete-row coverage, ranked missingness details, empty-column and high-cardinality text warnings, boolean flag summaries, duplicate-row and constant-column quality signals, categorical summaries, configurable ranked categorical values, numeric summaries that accept common currency/thousands formatting, ISO date-range summaries, numeric correlation detail that also supports business-formatted numeric columns, strongest-relationship highlights, and schema-warning signals for empty, duplicated, or uneven CSV structures, including stable placeholder names for blank headers. Its CLI also produces an optional standalone SVG missingness chart, with bundled examples and tests.
