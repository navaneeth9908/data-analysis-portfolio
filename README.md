...
The Auto EDA Analyst now profiles local CSV files into deterministic Markdown with a concise analyst summary, configurable validated one-character delimiters for non-comma exports, complete-row coverage, ranked missingness details, empty-column and high-cardinality text warnings, boolean flag summaries, duplicate-row and constant-column quality signals, categorical summaries, configurable ranked categorical values, numeric summaries that accept common currency/thousands formatting, ISO date-range summaries, numeric correlation detail that also supports business-formatted numeric columns, strongest-relationship highlights, and schema-warning signals for empty, duplicated, whitespace-padded, or uneven CSV structures, including stable placeholder names for blank headers. Its CLI also produces an optional standalone SVG missingness chart, with bundled examples and tests.
...
The Financial Research Analyst has deterministic local price-history data, return/risk metric calculations, aligned benchmark correlation/beta sensitivity, benchmark comparison, short-vs-long moving-average trend signals, volume-based liquidity profiles, optional trailing-twelve-month valuation/profitability snapshots, cross-period valuation and profitability comparisons, rule-backed risk notes, Markdown or self-contained HTML brief rendering, and tests for the CLI smoke path.

The Research Briefing Generator now provides a deterministic local JSON source-note contract, transparent source ranking with an explicit reporting date, publisher-coverage, freshness-band, and theme-mix summaries, source-coverage notes for thin or concentrated evidence, Markdown and self-contained HTML digest output with linked evidence and follow-up questions, a bundled sample, and end-to-end CLI tests.
