...
21. What is the repeat customer rate by region?
22. Which regions bought the widest product mix?
23. Show quarterly revenue by region for 2024
24. Show quarter over quarter revenue growth for 2024
...
32. How does product category revenue mix vary by customer segment?
...
For combined regional category revenue mix, run:

```bash
python -m src.offline_demo "How does product category revenue mix vary by region?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
region  | total_revenue | software_revenue | services_revenue | software_share_pct
--------+---------------+------------------+------------------+-------------------
West    | 6060.0        | 3710.0           | 2350.0           | 61.22
South   | 4900.0        | 2750.0           | 2150.0           | 56.12
Midwest | 2350.0        | 1150.0           | 1200.0           | 48.94
```

For customer-segment category revenue mix, run:

```bash
python -m src.offline_demo "How does product category revenue mix vary by customer segment?" --db-path examples/sales_mart.sqlite --limit 6
```

Expected result excerpt:

```text
segment            | total_revenue | software_revenue | services_revenue | software_share_pct
-------------------+---------------+------------------+------------------+-------------------
CPG                | 3910.0        | 2710.0           | 1200.0           | 69.31
Healthcare         | 2650.0        | 500.0            | 2150.0           | 18.87
Logistics          | 2350.0        | 1150.0           | 1200.0           | 48.94
```

For customer-segment software revenue share, run:

```bash
python -m src.offline_demo "Which customer segments have the highest software revenue share?" --db-path examples/sales_mart.sqlite --limit 6
```

Expected result excerpt:

```text
segment            | software_share_pct | total_revenue | software_revenue | services_revenue
-------------------+--------------------+---------------+------------------+-----------------
Financial Services | 100.0              | 2250.0        | 2250.0           | 0.0
CPG                | 69.31              | 3910.0        | 2710.0           | 1200.0
Logistics          | 48.94              | 2350.0        | 1150.0           | 1200.0
```

For customer-segment discount analysis, run:

```bash
python -m src.offline_demo "Which customer segments received the largest discounts?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
segment | discount_amount | discounted_units
--------+-----------------+-----------------
Retail  | 1000.0          | 1
CPG     | 40.0            | 1
```
...
For repeat-customer rate by region, run:

```bash
python -m src.offline_demo "What is the repeat customer rate by region?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
region    | total_customers | repeat_customers | repeat_customer_rate_pct
----------+-----------------+------------------+-------------------------
West      | 2               | 2                | 100.0
Midwest   | 1               | 1                | 100.0
South     | 2               | 1                | 50.0
...
For quarter-over-quarter revenue growth, run:

```bash
python -m src.offline_demo "Show quarter over quarter revenue growth for 2024" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
quarter | revenue | revenue_change | revenue_change_pct
--------+---------+----------------+-------------------
...
For customer-segment quarter-over-quarter growth, run:

```bash
python -m src.offline_demo "Which customer segments grew revenue quarter over quarter?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
segment   | quarter | revenue | previous_revenue | revenue_change | revenue_change_pct
----------+---------+---------+------------------+----------------+-------------------
Logistics | 2024-Q2 | 1200.0  | 1150.0           | 50.0           | 4.35
```

For product-level quarter-over-quarter growth, run:

```bash
python -m src.offline_demo "Which products grew revenue quarter over quarter?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
product_name       | category | quarter | revenue | previous_revenue | revenue_change | revenue_change_pct
-------------------+----------+---------+---------+------------------+----------------+-------------------
Data Quality Audit | Services | 2024-Q2 | 2400.0  | 1400.0           | 1000.0         | 71.43
```
