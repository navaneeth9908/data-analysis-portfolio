...
For customer-level discount analysis, run:

```bash
python -m src.offline_demo "Which customers received the largest discounts?" --db-path examples/sales_mart.sqlite --limit 10
```

Expected result excerpt:

```text
customer_name  | region | discount_amount | discounted_units | discounted_orders
---------------+--------+-----------------+------------------+------------------
