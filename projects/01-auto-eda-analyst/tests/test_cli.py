...
def test_cli_reports_negative_numeric_values_from_the_bundled_example(
    tmp_path: Path,
) -> None:
    source_file = Path("examples/sample_negative_values.csv")
    output_file = tmp_path / "eda_negative_values.md"
    environment = {**os.environ, "PYTHONPATH": str(Path.cwd() / "src")}
    ...
    assert "| refund_amount | 2 | -25.00 | -10.00 |" in report
